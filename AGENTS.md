...
- **Post-Task Review:** After successfully completing a discrete task on application code (`client/` or `server/`), **you must** invoke the **`code-refactor-reviewer`** subagent (`.cursor/agents/code-refactor-reviewer.md` or Task `subagent_type: code-refactor-reviewer`) to review changed files, apply safe refactors, write a timestamped report to `.cursor/reviews/`, and run `node .cursor/hooks/write-review-manifest.js <report-path>`. The `.cursor/hooks.json` `stop` hook (`.cursor/hooks/trigger-post-task-workflow.js`) auto-requests review when application files are dirty and not yet covered by `.cursor/reviews/.last-reviewed.json`.
- **Post-Task Push:** Only after the review manifest exists for the current change set, invoke the **`stable-build-git-push`** subagent (`.cursor/agents/stable-build-git-push.md` or Task `subagent_type: stable-build-git-push`) to verify the build, then stage, commit, and **push** to the current Git branch. The same stop hook auto-requests push once the manifest matches the current dirty files.
