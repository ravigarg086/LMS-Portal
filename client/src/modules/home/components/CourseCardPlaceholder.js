function CourseCardPlaceholder({ course }) {
  ...
  return (
    ...
    >
      <div className="card-img-top bg-primary-subtle d-flex align-items-center justify-content-center py-4">
        <span className="badge text-bg-primary fs-6">{course.id}</span>
      </div>
      <div className="card-body d-flex flex-column">
        <span className="badge text-bg-secondary align-self-start mb-2">{course.category}</span>
        <h3 id={titleId} className="card-title h5">
          {course.title}
        </h3>
